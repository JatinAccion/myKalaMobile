...
export class SearchResultComponent implements OnInit, DoCheck, OnDestroy {
    ...
    ngOnInit() {
        ...
        if (token == null && this.core.IsElasticSearch != true && this.core.IsGetoffers != true && this.core.IsBrowseProduct != true) /* When user not logged in */ {
            this.questionPopUps.push(new QuestionPopUp('Login'));
            this.questionPopUps.push(new QuestionPopUp('Join'));
    ...
    async browseESCUI() {
        ...
        var response = await this.core.searchProduct(text, parentName, 30, 0);
    ...
    selectShippingMethod(item, deliveryFee, deliveryMethod) {
        ...
        this.conversations.push(new Conversation(MsgDirection.Out, `${'$' + deliveryFee + ' - ' + deliveryMethod}`));
    ...
    async chargeAmount() {
        ...
        if (productAvailable) {
            this.conversations.push(new Conversation(MsgDirection.In, `${'Thanks, ' + this.getUserInfo.firstName + '! Glad you found what you want.'}`));
    ...
    loadShippingAddress() {
        this.checkout.getShippingAddress(JSON.parse(window.localStorage['userInfo']).emailId).subscribe((res) => {
            // this.conversation = new Conversation(MsgDirection.In, 'Please select your address or type in a new one.');
            // this.conversations.push(this.conversation);
        ...
        for (var i = 0; i < this.retiailerShipIds.length; i++) {
            ...
            for (var j = 0; j < this.itemsInCart.length; j++) {
                ...
                if (retId == item.retailerId) {
                    ...
                    filterItems.orderItems.push(new Orders(item.inStock, item.price, item.productDescription, item.productId, item.productImage, item.productName, item.quantity, item.shipProfileId, 0, item.taxCode, false))
    ...
    async getOffersReq(btnFlag) {
        if (window.localStorage['token'] == undefined) {
            this.conversations.push(new Conversation(MsgDirection.In, 'To create an offer request, please log in or sign up for an account'));
            ...
            if (searchProducts != undefined) {
                ...
                if (this.core.isSearchWithoutSuggestion) this.getPlaces();
                    ...
                    //this.getSubCategory();
                    if (this.Step1SelectedValues.subcategory == null || this.Step1SelectedValues.subcategory == "") {
                        this.getSubCategory();
                        this.getUpdateTypes();
                        this.questionPopUps = [];
            ...
            case 'Skip':
                {
                    ...
                    if (this.showInterestPreference) {
                        this.conversations.push(new Conversation(MsgDirection.In, "Your all done! Let's get shopping!"));
                        this.core.deactivateRouteFlag(false);
                        setTimeout(() => { this.router.navigateByUrl('/home') }, 2000);
                        this.scrollTo();
        ...
        if (from == 'signup') {
            ...
            if (res.userId != null || res.userId != undefined) {
                if (res.userCreateStatus) {
                    ...
                    this.fpModal.resetLink = 'http://consumer-qa.mykala.s3-website.us-east-2.amazonaws.com' + `/#/reset-password?id=${userId}`;
    ...
    async addChatInput(userInputChat: any) {
        ...
        this.UserInput = "";
        if (this.conversations[this.conversations.length - 2] != undefined && this.conversations[this.conversations.length - 4] != undefined && (this.conversations[this.conversations.length - 2].data == "Login" || (this.conversations[this.conversations.length - 4].data == "Login" && this.conversations[this.conversations.length - 1].data == "Password too!"))) {
            ...
            if (this.conversations[this.conversations.length - 1] != undefined) {
                if (this.conversations[this.conversations.length - 1].data == 'Can I have your first name?') {
                    ...
                    if (this.Step3Modal.getoffer_3.length == 0) {
                        this.Step3Data = "";
                        this.Step4Summary = { ...this.Step1Modal.getoffer_1[0], ...this.GetOfferStep_2, ...this.Step3Modal.getoffer_3 };
                        this.showGetOfferSummary = true;
                    }
                    else if (this.Step3Modal.getoffer_3 != undefined) {

                        this.Step4Summary = { ...this.Step1Modal.getoffer_1[0], ...this.GetOfferStep_2, ...this.Step3Modal.getoffer_3[0] };
                        this.showGetOfferSummary = true;
                    }
                    //get the get offer summary
                    ...
                    if (this.GetOfferStep_2Summary.attributes[this.lastValueForAPI].findIndex(i => i == "Other") != -1) {
                        this.GetOfferStep_2Summary.attributes[this.lastValueForAPI].splice(i => i == "Other", 1);
                    }
    ...
    selectShippingAddress(e, address) {
        ...
        for (var i = 0; i < this.itemsInCart.length; i++) {
            this.checkout.getShippingMethods(address.state, this.itemsInCart[i].shipProfileId, this.itemsInCart[i].quantity, this.itemsInCart[i].shippingWeight, this.itemsInCart[i].price, this.itemsInCart[i].shippingLength, this.itemsInCart[i].shippingHeight, this.itemsInCart[i].shippingWidth).subscribe((res) => {
                ...
                for (var i = 0; i < res.deliveryMethods.length; i++) res.deliveryMethods[i].deliveryFee = res.deliveryMethods[i].deliveryFee + res.locationFee;
    ...
    async onSubmitAddCard(form: NgForm) {
        if (this.getUserInfo === undefined) this.error = "Please login to add new card";
            ...
            if (!this.addPaymentCard.touched) this.error = 'All fields are mandatory';
            else {
    ...
    deleteNoPreferenceType() {
        if (this.userResponse.type.length > 0) {
            if (this.userResponse.type[3].name == "No Preference" && this.RemoveNoPreferenceType) {
