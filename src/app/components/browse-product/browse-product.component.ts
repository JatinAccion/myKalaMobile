...
export class BrowseProductComponent implements OnInit {
  ...
  clearSubcategory: boolean = true;

  constructor(private homeService: HomeService, private http: Http, public core: CoreService, private route: Router, private changeDetRef: ChangeDetectorRef, private ng4LoadingSpinnerService: Ng4LoadingSpinnerService) { }
  ...
  loadTypes(from?: any) {
    ...
    if (this.clearSubcategory) {
      let removeSubCategory = JSON.parse(window.localStorage['levelSelections']);
      removeSubCategory.subcategory = [];
      window.localStorage['levelSelections'] = JSON.stringify(removeSubCategory);
    }
  ...
  filterResponse(res) {
    ...
    document.getElementById("categoryHdr") != null ? document.getElementById("categoryHdr").style.height = "0px" : {};
  }
  ...
  viewDetails(tile) {
    ...
    if (tile.product.phGetOffers) {
      let subcat = tile.product.phGetOffers.find(element =>
        element.levelName == tile.product.productSubCategoryName);
