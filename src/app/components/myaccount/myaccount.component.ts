...
export class MyaccountComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  getConsumerProfile() {
    ...
    this.myAccount.getUserDetails(userId).subscribe((res) => {
      ...
      this.myAccountModel.profileInfo.address = res.address == null ? [] : res.address;
  ...
  fileChangeEvent(fileInput: any) {
    if (fileInput.target.files && fileInput.target.files[0]) {
      ...
      setTimeout(() => {
        this.mockProfileImage = window.localStorage['imagesPath'];
  ...
  _keuyp(e) {
    ...
    if (this.changeLocationForm.controls.location.value.toString().length == 5) {
      this.loader = true;
      this.errorValidationMessage = '';
      input.setAttribute('readonly', true);
      this.myAccount.getLocation(this.changeLocationForm.controls.location.value)
        .subscribe(data => {
          this.loader = false;
  ...
  saveChange(from) {
    if (from == 'profileImage') {
      ...
      if (!this.addShippingAddressForm.controls.addressLineOne.value) this.errorValidationMessage = 'Please enter Address Line 1';
        ...
        this.errorValidationMessage = '';
        this.loader_shippingAddress = true;
        this.myAccountModel.profileInfo.address.push(new MyAccountAddress(null, this.addShippingAddressForm.controls.addressLineOne.value, this.addShippingAddressForm.controls.addressLineTwo.value, this.addShippingAddressForm.controls.addCity.value, this.addShippingAddressForm.controls.addStates.value, this.addShippingAddressForm.controls.addZipcode.value.toString(), 'shippingAddress'));
        this.AddressSaveModel.emailId = this.getUserInfo.emailId;
        this.AddressSaveModel.address = this.myAccountModel.profileInfo.address;
        this.myAccount.saveAddress(this.AddressSaveModel).subscribe((res) => {
          this.loader_shippingAddress = false;
